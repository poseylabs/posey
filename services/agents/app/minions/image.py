...
class ImageMinion(BaseMinion):
    """Image generation minion"""
    ...
    def setup(self):
        ...
        logger.debug(f"System prompt configured for image minion")

        # Create agent for prompt optimization
        self.agent = self.create_agent(result_type=ImageRequest, model_key="reasoning")
        logger.info(f"Image agent created with model: {LLM_CONFIG['reasoning']['model']}")

    def create_prompt_context(self, context: Dict[str, Any], image_data: Dict[str, Any] = None) -> BasePromptContext:
        """Create a properly structured prompt context for image operations
        
        Args:
            context: The raw context dictionary
            image_data: Additional image-specific data to include in the context
    ...
    async def execute(self, params: Dict[str, Any], context: RunContext) -> Dict[str, Any]:
        """Execute image minion operations"""
        start_time = time.time()
        ...
        try:
            ...
            logger.error(f"Error in image minion execute: {str(e)}")
            ...
            return {
                "status": "error",
                "error": str(e),
                "error_type": "image_minion_exception",
                ...
                "metadata": {
                    ...
                    "execution_time": time.time() - start_time,
                    "minion": "image",
                    "parameters": {k: v for k, v in params.items() if k != "prompt"} # Exclude prompt to avoid leaking sensitive info
                }
            }
