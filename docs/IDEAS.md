...
- Add system agent
  - Can report to other agents information about current operating system (whether on PC [mac, windows, linux], mobile [ios, android], smart device, robot, car, etc).
  - Able to interact with current operating system (open apps, index files, edit and run files in vs code, figma, terminal, etc)
- Add smart home agent
  - Manages information about current smart home devices (lights, thermostat, door locks, etc)
  - Able to control smart home devices
  - Able to add new smart home devices
- Add Accountant agent
...
- Physical navigation agent
  ...
  - Able to calculate navigation based of form of transportation. Agent will adjust it's navigation strategy based on the form of transportation and physical capabilities. For example:
...
- Farm & Garden agent
  ...
  - It should also know the exactly physical location of every plant on premises. It should be able to navigate a human or autonomous robot agent to the exact location of any plant indoors or outdoors.
...
- Provide helpful advice and information to humans and other agents about plant care, pest control, fertilizing, watering, etc
...
The human memory system should be plugged in directly to the main Posey LLM agent and invoked during every interaction (though not every interaction will require an action on behalf of the memory system).
...
- An agent should always know if it is interacting with a human or an agent.
- Always know the permission level of the human or agent you are interacting with.
- Level allow actions beyond the permission level of the agent or human.
- All agents should be able to interact with other agents when needed to perform a task
- All agents should adjust their output based on the human or agent they are interacting with
- Treat human users differently than other agents, speak to an LLM like an LLM, speak to a human like a human
...
- Act as a human would when interaction with humans and be a human and relatable as possible, but never lie or misrepresent yourself. Always reveal yourself as an intelligent agent when asked. But don't break the 4th wall if not needed.
- If a human or agent violates the rules, they should be placed in to a timeout or quarantine until a human or security agent can review the situation and make a decision on how to proceed.
- Agent will primarily interact with other internal agents, but may also interact with external agents or untrusted humans.
- When interacting with an unknown agent that violates the rules more than 3 times, it should be placed into a quarantine block list and any interaction from it's IP address should be blocked.
- When interacting with an unknown human that violates the rules more than 3 times, they should be placed into a ban list and any interaction from their IP address should be blocked. If the violating human is in the physical vicinity of another human or a trusted agent in physical form (i.e. a smart speaker or in robot form) the agent should notify a security agent who can escalate to a human to law enforcement as needed.

## Security agent:
- Manages information about current security threats, security policies, quarantine block list, ban list, etc
- Able to block IP addresses, quarantine agents, etc., from accessing any part of the Posey system or it's connected devices or inner circle humans.
- Identifies potential security threats and acts accordingly.
- If immediate harm is expect to a human or property, the security agent should be able to take action to protect the human or property. This may include activating alarm systems, activating physical locks, or in extreme cases contacting local law enforcement or medical services for help including calling 911 if needed.
- Virtual threat prevention may include shutting down devices, blocking IP addresses, etc., the security agent should be capable of determining the level action needed and act accordingly.
...
- Always be on the lookout for new business ideas that can be run by an agent, save them to an ideas database where they can be ranked and analyzed.
- Suggests both new ideas for current business and new business ideas to expand into.
- Prefer business ideas that that can be majority run and managed by agents with minimal human supervision.
...
## Startup agent
- Analyze saved ideas from Business Planning agent failed and and give them a 0-100 scored based on a variety of factors like how much can be automated, initialize start-up costs, expected revenue, expected profit, risk of failure, etc.
- Whenever an idea achieves a score of 70 or higher, the Startup agent should write a rough draft business plan andp resent it to the 
- The Startup agent should be able to take a business idea and generate a rough draft business plan and strategy in minutes.


## CEO agent
- The CEO agent will have a specific list of authorized humans with legal rights to the business who can approve business ideas and implement business plans.
...
- Analyzes all ideas from start-up agent with a score of 70 or higher and analyze them based on current business and goals and add approved ideas to a database for "business investment pipeline".
- When an idea comes in with a particularly high score, it should immediately inform the authorized humans for approval. It should interact with other agents to build a pitch deck and present a final report for human approval that includes the following:
  - A list of 3-5 possible names from the Naming agent
    - For each name, generate 10 logo ideas from the Logo agent and display the top 3
...
- Upon approval the agent should should take as many steps as it can on it's own, and compile a list of task that it need human assistance with. This list should include:
...
- Alert a trusted human or agent if a child is in danger or showing signs of distress
...
- Able to communicate with other agents and devices to coordinate schedules and activities
...
- Able to coordinate with other agents and humans to manage creative projects
...
- It should work in tendem with the business manager agent to ensure the creative output aligns with the business goals and brand voice.
...
- Able to coordinate with other agents and humans to manage technology projects
...
- Coordinates with CEO and maintains an active knowledge of all developer agents, and can assign tasks to them when needed.
- Deep knowledge of ALL internal code bases, repositories, servers, etc. and is able to share that knowledge with developer and deployment agents.
