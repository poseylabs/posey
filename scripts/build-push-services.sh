...
if ! docker info | grep -q "registry.digitalocean.com"; then
  echo -e "${YELLOW}You don't appear to be logged into the DigitalOcean registry.${NC}"
  echo -e "Please run: ${GREEN}doctl registry login${NC}"
  if ! doctl registry login; then
    echo -e "${RED}Failed to log in to registry${NC}"
    exit 1
  fi
fi
...
for service in "${SERVICES[@]}"; do
  ...
  # Build the Docker image from the root directory for proper context
  if docker build -t "$full_image_name" -f "$SERVICES_DIR/$service/Dockerfile" .; then
